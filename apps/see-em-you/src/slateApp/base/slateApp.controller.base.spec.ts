...
const service = {
  ...
  slateApps: () => FIND_MANY_RESULT,
  slateApp: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("SlateApp", () => {
  ...
  test("GET /slateApps/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/slateApps"}/${nonExistingId}`)
  ...
  test("GET /slateApps/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/slateApps"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect(FIND_ONE_RESULT);
  });

  test("POST /slateApps existing resource", async () => {
