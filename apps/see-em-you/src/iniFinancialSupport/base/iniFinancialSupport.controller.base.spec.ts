...
describe("IniFinancialSupport", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: IniFinancialSupportService,
          useValue: service,
        },
      ],
  ...
  test("POST /iniFinancialSupports", async () => {
    await request(app.getHttpServer())
      .post("/iniFinancialSupports")
      .send(CREATE_INPUT)
      .expect(HttpStatus.CREATED)
      .expect(CREATE_RESULT);
  });

  test("GET /iniFinancialSupports", async () => {
    await request(app.getHttpServer())
      .get("/iniFinancialSupports")
      .expect(HttpStatus.OK)
      .expect([FIND_MANY_RESULT[0]]);
  });

  test("GET /iniFinancialSupports/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/iniFinancialSupports"}/${nonExistingId}`)
  ...
  test("GET /iniFinancialSupports/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/iniFinancialSupports"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect(FIND_ONE_RESULT);
  });

  test("POST /iniFinancialSupports existing resource", async () => {
