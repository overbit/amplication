/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { MhciPrereqsProgrammingSampleWhereUniqueInput } from "./MhciPrereqsProgrammingSampleWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { MhciPrereqsProgrammingSampleUpdateInput } from "./MhciPrereqsProgrammingSampleUpdateInput";

@ArgsType()
class UpdateMhciPrereqsProgrammingSampleArgs {
  @ApiProperty({
    required: true,
    type: () => MhciPrereqsProgrammingSampleWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => MhciPrereqsProgrammingSampleWhereUniqueInput)
  @Field(() => MhciPrereqsProgrammingSampleWhereUniqueInput, {
    nullable: false,
  })
  where!: MhciPrereqsProgrammingSampleWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => MhciPrereqsProgrammingSampleUpdateInput,
  })
  @ValidateNested()
  @Type(() => MhciPrereqsProgrammingSampleUpdateInput)
  @Field(() => MhciPrereqsProgrammingSampleUpdateInput, { nullable: false })
  data!: MhciPrereqsProgrammingSampleUpdateInput;
}

export { UpdateMhciPrereqsProgrammingSampleArgs as UpdateMhciPrereqsProgrammingSampleArgs };
