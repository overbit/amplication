...
const service = {
  createSlateRecommend1() {
    return CREATE_RESULT;
  },
  slateRecommend1s: () => FIND_MANY_RESULT,
  slateRecommend1: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("SlateRecommend1", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: SlateRecommend1Service,
          useValue: service,
        },
      ],
  ...
  test("POST /slateRecommend1s", async () => {
    await request(app.getHttpServer())
      .post("/slateRecommend1s")
      .send(CREATE_INPUT)
      .expect(HttpStatus.CREATED)
      .expect(CREATE_RESULT);
  });

  test("GET /slateRecommend1s", async () => {
    await request(app.getHttpServer())
      .get("/slateRecommend1s")
      .expect(HttpStatus.OK)
      .expect([FIND_MANY_RESULT[0]]);
  });

  test("GET /slateRecommend1s/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/slateRecommend1s"}/${nonExistingId}`)
  ...
  test("GET /slateRecommend1s/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/slateRecommend1s"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect(FIND_ONE_RESULT);
  });

  test("POST /slateRecommend1s existing resource", async () => {
