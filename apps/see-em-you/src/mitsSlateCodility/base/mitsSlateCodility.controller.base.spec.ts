...
describe("MitsSlateCodility", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: MitsSlateCodilityService,
          useValue: service,
        },
      ],
  ...
  test("POST /mitsSlateCodilities", async () => {
    await request(app.getHttpServer())
      .post("/mitsSlateCodilities")
      .send(CREATE_INPUT)
      .expect(HttpStatus.CREATED)
      .expect(CREATE_RESULT);
  });

  test("GET /mitsSlateCodilities", async () => {
    await request(app.getHttpServer())
      .get("/mitsSlateCodilities")
      .expect(HttpStatus.OK)
      .expect([FIND_MANY_RESULT[0]]);
  });

  test("GET /mitsSlateCodilities/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/mitsSlateCodilities"}/${nonExistingId}`)
  ...
  test("GET /mitsSlateCodilities/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/mitsSlateCodilities"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect(FIND_ONE_RESULT);
  });

  test("POST /mitsSlateCodilities existing resource", async () => {
