/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CashnetPaymentCopyWhereInput } from "./CashnetPaymentCopyWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class CashnetPaymentCopyListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => CashnetPaymentCopyWhereInput,
  })
  @ValidateNested()
  @Type(() => CashnetPaymentCopyWhereInput)
  @IsOptional()
  @Field(() => CashnetPaymentCopyWhereInput, {
    nullable: true,
  })
  every?: CashnetPaymentCopyWhereInput;

  @ApiProperty({
    required: false,
    type: () => CashnetPaymentCopyWhereInput,
  })
  @ValidateNested()
  @Type(() => CashnetPaymentCopyWhereInput)
  @IsOptional()
  @Field(() => CashnetPaymentCopyWhereInput, {
    nullable: true,
  })
  some?: CashnetPaymentCopyWhereInput;

  @ApiProperty({
    required: false,
    type: () => CashnetPaymentCopyWhereInput,
  })
  @ValidateNested()
  @Type(() => CashnetPaymentCopyWhereInput)
  @IsOptional()
  @Field(() => CashnetPaymentCopyWhereInput, {
    nullable: true,
  })
  none?: CashnetPaymentCopyWhereInput;
}
export { CashnetPaymentCopyListRelationFilter as CashnetPaymentCopyListRelationFilter };
