...
describe("MitsSlateRecommend3", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: MitsSlateRecommend3Service,
          useValue: service,
        },
      ],
  ...
  test("POST /mitsSlateRecommend3s", async () => {
    await request(app.getHttpServer())
      .post("/mitsSlateRecommend3s")
      .send(CREATE_INPUT)
      .expect(HttpStatus.CREATED)
      .expect(CREATE_RESULT);
  });

  test("GET /mitsSlateRecommend3s", async () => {
    await request(app.getHttpServer())
      .get("/mitsSlateRecommend3s")
      .expect(HttpStatus.OK)
      .expect([FIND_MANY_RESULT[0]]);
  });

  test("GET /mitsSlateRecommend3s/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/mitsSlateRecommend3s"}/${nonExistingId}`)
  ...
  test("GET /mitsSlateRecommend3s/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/mitsSlateRecommend3s"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect(FIND_ONE_RESULT);
  });

  test("POST /mitsSlateRecommend3s existing resource", async () => {
