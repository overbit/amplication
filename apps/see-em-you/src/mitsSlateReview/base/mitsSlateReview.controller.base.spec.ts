...
const CREATE_INPUT = {
  ...
  mseReviewFormNum1MitsEvaluation: "exampleMseReviewFormNum1MitsEvaluation",
  ...
  mseReviewFormNum2MitsEvaluation: "exampleMseReviewFormNum2MitsEvaluation",
  ...
  mseReviewFormNum3MitsEvaluation: "exampleMseReviewFormNum3MitsEvaluation",
  ...
  mseReviewFormNum4MitsEvaluation: "exampleMseReviewFormNum4MitsEvaluation",
  ...
  mseReviewFormNum5MitsEvaluation: "exampleMseReviewFormNum5MitsEvaluation",
...
const service = {
  createMitsSlateReview() {
    return CREATE_RESULT;
  },
  mitsSlateReviews: () => FIND_MANY_RESULT,
  mitsSlateReview: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("MitsSlateReview", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: MitsSlateReviewService,
          useValue: service,
        },
      ],
  ...
  test("POST /mitsSlateReviews", async () => {
    await request(app.getHttpServer())
      .post("/mitsSlateReviews")
      .send(CREATE_INPUT)
      .expect(HttpStatus.CREATED)
      .expect(CREATE_RESULT);
  });

  test("GET /mitsSlateReviews", async () => {
    await request(app.getHttpServer())
      .get("/mitsSlateReviews")
      .expect(HttpStatus.OK)
      .expect([FIND_MANY_RESULT[0]]);
  });

  test("GET /mitsSlateReviews/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/mitsSlateReviews"}/${nonExistingId}`)
  ...
  test("GET /mitsSlateReviews/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/mitsSlateReviews"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect(FIND_ONE_RESULT);
  });

  test("POST /mitsSlateReviews existing resource", async () => {
