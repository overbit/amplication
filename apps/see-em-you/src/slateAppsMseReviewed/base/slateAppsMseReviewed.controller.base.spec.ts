...
const service = {
  createSlateAppsMseReviewed() {
    return CREATE_RESULT;
  },
  slateAppsMseRevieweds: () => FIND_MANY_RESULT,
  slateAppsMseReviewed: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("SlateAppsMseReviewed", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: SlateAppsMseReviewedService,
          useValue: service,
        },
      ],
  ...
  test("POST /slateAppsMseRevieweds", async () => {
    await request(app.getHttpServer())
      .post("/slateAppsMseRevieweds")
      .send(CREATE_INPUT)
      .expect(HttpStatus.CREATED)
      .expect(CREATE_RESULT);
  });

  test("GET /slateAppsMseRevieweds", async () => {
    await request(app.getHttpServer())
      .get("/slateAppsMseRevieweds")
      .expect(HttpStatus.OK)
      .expect([FIND_MANY_RESULT[0]]);
  });

  test("GET /slateAppsMseRevieweds/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/slateAppsMseRevieweds"}/${nonExistingId}`)
  ...
  test("GET /slateAppsMseRevieweds/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/slateAppsMseRevieweds"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect(FIND_ONE_RESULT);
  });

  test("POST /slateAppsMseRevieweds existing resource", async () => {
