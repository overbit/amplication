...
const service = {
  createMitsSlateSuppinfo() {
    return CREATE_RESULT;
  },
  mitsSlateSuppinfos: () => FIND_MANY_RESULT,
  mitsSlateSuppinfo: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("MitsSlateSuppinfo", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: MitsSlateSuppinfoService,
          useValue: service,
        },
      ],
  ...
  test("POST /mitsSlateSuppinfos", async () => {
    await request(app.getHttpServer())
      .post("/mitsSlateSuppinfos")
      .send(CREATE_INPUT)
      .expect(HttpStatus.CREATED)
      .expect(CREATE_RESULT);
  });

  test("GET /mitsSlateSuppinfos", async () => {
    await request(app.getHttpServer())
      .get("/mitsSlateSuppinfos")
      .expect(HttpStatus.OK)
      .expect([FIND_MANY_RESULT[0]]);
  });

  test("GET /mitsSlateSuppinfos/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/mitsSlateSuppinfos"}/${nonExistingId}`)
  ...
  test("GET /mitsSlateSuppinfos/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/mitsSlateSuppinfos"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect(FIND_ONE_RESULT);
  });

  test("POST /mitsSlateSuppinfos existing resource", async () => {
