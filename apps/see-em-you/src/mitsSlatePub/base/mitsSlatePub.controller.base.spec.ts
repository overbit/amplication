...
const service = {
  ...
  mitsSlatePubs: () => FIND_MANY_RESULT,
  mitsSlatePub: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("MitsSlatePub", () => {
  ...
  test("GET /mitsSlatePubs/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/mitsSlatePubs"}/${nonExistingId}`)
  ...
  test("GET /mitsSlatePubs/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/mitsSlatePubs"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect(FIND_ONE_RESULT);
  });

  test("POST /mitsSlatePubs existing resource", async () => {
