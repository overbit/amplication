/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DomainUnitWhereUniqueInput } from "./DomainUnitWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { DomainUnitUpdateInput } from "./DomainUnitUpdateInput";

@ArgsType()
class UpdateDomainUnitArgs {
  @ApiProperty({
    required: true,
    type: () => DomainUnitWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => DomainUnitWhereUniqueInput)
  @Field(() => DomainUnitWhereUniqueInput, { nullable: false })
  where!: DomainUnitWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => DomainUnitUpdateInput,
  })
  @ValidateNested()
  @Type(() => DomainUnitUpdateInput)
  @Field(() => DomainUnitUpdateInput, { nullable: false })
  data!: DomainUnitUpdateInput;
}

export { UpdateDomainUnitArgs as UpdateDomainUnitArgs };
