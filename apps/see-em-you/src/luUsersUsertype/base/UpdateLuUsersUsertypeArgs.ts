/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { LuUsersUsertypeWhereUniqueInput } from "./LuUsersUsertypeWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { LuUsersUsertypeUpdateInput } from "./LuUsersUsertypeUpdateInput";

@ArgsType()
class UpdateLuUsersUsertypeArgs {
  @ApiProperty({
    required: true,
    type: () => LuUsersUsertypeWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => LuUsersUsertypeWhereUniqueInput)
  @Field(() => LuUsersUsertypeWhereUniqueInput, { nullable: false })
  where!: LuUsersUsertypeWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => LuUsersUsertypeUpdateInput,
  })
  @ValidateNested()
  @Type(() => LuUsersUsertypeUpdateInput)
  @Field(() => LuUsersUsertypeUpdateInput, { nullable: false })
  data!: LuUsersUsertypeUpdateInput;
}

export { UpdateLuUsersUsertypeArgs as UpdateLuUsersUsertypeArgs };
