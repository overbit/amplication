/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { RecommenderInfoWhereInput } from "./RecommenderInfoWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { RecommenderInfoOrderByInput } from "./RecommenderInfoOrderByInput";

@ArgsType()
class RecommenderInfoFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => RecommenderInfoWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => RecommenderInfoWhereInput, { nullable: true })
  @Type(() => RecommenderInfoWhereInput)
  where?: RecommenderInfoWhereInput;

  @ApiProperty({
    required: false,
    type: [RecommenderInfoOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [RecommenderInfoOrderByInput], { nullable: true })
  @Type(() => RecommenderInfoOrderByInput)
  orderBy?: Array<RecommenderInfoOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { RecommenderInfoFindManyArgs as RecommenderInfoFindManyArgs };
